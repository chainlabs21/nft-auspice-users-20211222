...
/*	function mint_begin_simple_and_bid (
	...
	const on_bid_to_auction = async _=>{
		...
		let aargs = [
				ADDRESSES.erc1155 // 0
			, sellorder?.username // 1
			, sellorder?.itemid // 2
			, itemdata?.item?.countcopies  // 3
			, itemdata?.item?.authorfee // 4
			, tokenid || '0' // 5
			, sellorder?.asset_amount_bid // 6
			, getweirep( sellorder?.asset_amount_ask ) // 7
			, sellorder?.startingtime? sellorder?.startingtime : moment().unix() // 8
			, sellorder?.expiry // 9
			, getweirep( mybidamount )  // 10
		]
		let abistr = getabistr_forfunction ( {
			contractaddress  : ADDRESSES.auction_repo_english_simple // auction_repo_english_simple_no_batch_tasks
...
//		return 
	...
	const onclickbuy = _ =>{
		switch ( sellorder?.typestr){
			...
			break
			case 'AUCTION_ENGLISH' : on_bid_to_auction ()
			break
